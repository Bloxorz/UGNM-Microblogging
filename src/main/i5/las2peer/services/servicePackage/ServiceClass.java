...
public class ServiceClass extends Service {
	 ...
	 */
	@POST
	@Path("user/{idUser}/setPassword")
	public HttpResponse setUserPassword(@PathParam("idUser") String idUser, @ContentParam String content) {
		...
		ResultSet rs = null;
		try {
			JSONObject jso = (JSONObject) JSONValue.parseWithException(content);
			String pass = jso.values().toArray()[0].toString();
			conn = dbm.getConnection();
			stmnt = conn.prepareStatement("UPDATE User SET pass = ? WHERE idUser = ?;");
			stmnt.setString(1, pass);
			stmnt.setString(2, idUser);
			int rows = stmnt.executeUpdate(); // same works for insert
			result = "Database updated. " + rows + " rows affected";
